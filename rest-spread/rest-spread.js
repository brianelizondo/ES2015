...
function filterOutOdds(...oddsArray){
    return oddsArray.filter((odd, nextVal) => nextVal % 2 === 0);
}
...
// findMin(1,-1) // -1
// findMin(3,1) // 1
const findMin = (...arrayValues) => Math.min(...arrayValues);
...
const doubleAndReturnArgs = (array1, ...args) => [...array1, ...args.map(value => value * 2)];
...
const removeRandom = items => {
    let idxRandom = Math.random() * items.length;
    idxRandom = Math.floor(idxRandom);
